...
userRouter.use((req, res, next) => {
  ...
  <a href="/add">Créer un utilisateur</a> |
  <a href="/update">Modifier un utilisateur</a> |
  <a href="/delete">Supprimer un utilisateur</a>
</div>
  `;
...
userRouter.get('/', (req, res) => {
    ...
    <html>
      ...
      <div style="display: flex; justify-content: center; align-items: center; height: 100vh; background-color: rgba(0, 0, 0, 0);">
        <div id="notification" style="background-color: #f3f4f6; padding: 2rem; border-radius: 0.5rem; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);">
          ...
          <form style="width: 300px;" action="/add" method="post" onsubmit="return validateForm()">
  ...
  userRouter.post('/add', (req, res) => {
    userController.create(req.body, (err, result) => {
      if (err) {
        return res.redirect('/add?error=true');
      }
      return res.redirect('/add?success=true');
    });
  });
...
userRouter.post('/user/update/:username', (req, res) => {
  ...
  userController.updateUser(username, updatedData, (err, result) => {
      if (err) {
